...
selected={uid(pad) for f in b.GetFootprints() if f.GetReference()=='U12' for pad in f.Pads()}
seen=set();groups=[]
for item in allitems:
 if item.GetNetname() not in ['GND','VDDA'] or uid(item) in seen:continue
 if uid(item) not in selected:continue
 group=[];todo=[item]
 while todo:
  a=todo.pop();k=uid(a)
...
def safe_track(a,z,net):
 ...
 tr=p.PCB_TRACK(b);tr.SetStart(a);tr.SetEnd(z);tr.SetWidth(p.FromMM(.15));tr.SetLayer(p.F_Cu)
...
for g in groups:
 ...
 if a!=pos:
  tr=p.PCB_TRACK(b);tr.SetStart(a);tr.SetEnd(pos);tr.SetWidth(p.FromMM(.15));tr.SetLayer(p.F_Cu);tr.SetNetCode(net);b.Add(tr);shapes.append((tr,tr.GetEffectiveShape()))
